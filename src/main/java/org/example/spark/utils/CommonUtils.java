...
public class CommonUtils {
    ...
    public static void traceColumn(Dataset<Row> df, Map<String, List<TraceColumn>> columnTraceMap,Map<String, List<TraceColumn>> previousColumnTrace ){
        ...
        for (String column: df.columns()){
            ...
            if(!ObjectUtils.isEmpty(previousColumnTrace) &&  previousColumnTrace.containsKey(column)){
                List<TraceColumn> prevList = previousColumnTrace.get(column);
                List<TraceColumn> currList = columnTraceMap.get(column);
                currList.addAll(prevList);
                columnTraceMap.put(column,currList);
